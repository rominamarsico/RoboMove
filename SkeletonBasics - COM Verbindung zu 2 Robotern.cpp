...
public:
	...
	int connect2(); //neu
	int connect2(wchar_t *COM21); //neu
	...
	int sendArray2(unsigned char *buffer2, int len2);	//neu
	int getArray2(unsigned char *buffer2, int len2); //neu
...
//neu Anfang
int SerialPort::connect2(wchar_t* device2) {
	int error = 0;
	DCB dcb;
	...
	serialPortHandle = CreateFile(device2, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, NULL, NULL);
...
//neu Anfang
int SerialPort::sendArray2(unsigned char *buffer2, int len2) {
	unsigned long result;
	//cout << "test";

	if (serialPortHandle != INVALID_HANDLE_VALUE)
		WriteFile(serialPortHandle, buffer2, len2, &result, NULL);

	return result;
}

int SerialPort::getArray2(unsigned char *buffer2, int len2) {
	unsigned long read_nbr;

	read_nbr = 0;
	if (serialPortHandle != INVALID_HANDLE_VALUE)
	{
		ReadFile(serialPortHandle, buffer2, len2, &read_nbr, NULL);
	}
